...
`lehrlings-core` contains only **interfaces + pure compute functions** (e.g. dovetail geometry). Implementations live in consumer apps. The included `boundary-check.sh` script enforces this — no framework imports, no auth/db imports.
...
```typescript
import { ModeManager } from "@voai/lehrlings-core";
...
🚧 **0.1.0** — Initial release. Ships with the standalone app `@voai/lehrling-edu` (Phase B + C + D voice stack ready, dormant without API keys).
